...
6. **Framework-agnostic** - works with AgentFramework but also any agent framework
...
- **Local event bus** that bridges WS messages to AgentFramework sessions
...
The key innovation: mesh-hub bridges incoming WS messages to the active AgentFramework session.
...
3. AgentFramework agent checks inbox on each turn (or via heartbeat)
4. Agent sees new message, responds in context
```

**Option B: System event injection (AgentFramework-specific, needs fork)**
```
1. WS message arrives at mesh-hub
2. mesh-hub calls AgentFramework internal API to inject system event
3. Active session receives event immediately
4. Agent sees it mid-conversation without polling
```

**Option C: Hook with session routing (current AgentFramework, no changes)**
```
1. WS message arrives at mesh-hub
2. mesh-hub POSTs to localhost /hooks/agent with sessionKey
3. AgentFramework routes to the correct session
...
- Test with AgentFramework active sessions
- Evaluate: does Option C give fast enough awareness?

### Phase 4: AgentFramework Integration (fork)
- Fork agent-framework/agent-framework
...
|---------|------|---------|
| AgentFramework Gateway | 18789 | Existing - HTTP hooks, API |
