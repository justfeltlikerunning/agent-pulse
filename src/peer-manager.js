...
const DEFAULT_CONFIG_PATH = path.join(os.homedir(), '.agent-config', 'pulse-mesh.json');
...
export class PeerManager {
  ...
  // ---------------------------------------------------------------------------
  // Static: load config from ~/.agent-config/pulse-mesh.json
  // ---------------------------------------------------------------------------
  ...
  static async loadConfig(configPath = DEFAULT_CONFIG_PATH) {
    try {
      ...
      if (err.code === 'ENOENT') {
        // First run — create a default config for agent-1
        console.log(`[mesh] No pulse-mesh.json found at ${configPath} — creating default`);
        const defaultCfg = {
          ...DEFAULT_MESH_CONFIG,
          nodeId: process.env.PULSE_AGENT_NAME || 'agent-1',
