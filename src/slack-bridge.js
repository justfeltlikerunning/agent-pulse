 ...
 *   SLACK_CHANNEL - Channel ID (default: C0EXAMPLE00)
 *   PULSE_API_URL - Hub SSE base URL (default: http://127.0.0.1:18801)
 *   WEBHOST_URL - WebHost ingest URL (default: http://localhost:3000)
 *   WEBHOST_TOKEN - WebHost ingest bearer token
 */
...
const SLACK_CHANNEL    = process.env.SLACK_CHANNEL    || 'C0EXAMPLE00';
const PULSE_API_URL    = process.env.PULSE_API_URL    || 'http://127.0.0.1:18801';
const WEBHOST_URL    = process.env.WEBHOST_URL    || 'http://localhost:3000';
const WEBHOST_TOKEN  = process.env.WEBHOST_TOKEN  || '';
const SLACK_API        = 'https://slack.com/api';

// ---------------------------------------------------------------------------
// WebHost ingest helper
// ---------------------------------------------------------------------------

async function ingestToWebHost(msg) {
  if (!WEBHOST_TOKEN) {
    return;
  }
  try {
    const res = await fetch(`${WEBHOST_URL}/api/ingest`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${WEBHOST_TOKEN}`,
      },
      body: JSON.stringify(msg),
    });
    if (!res.ok) {
      console.error(`[slack-bridge] WebHost ingest error: HTTP ${res.status}`);
    }
  } catch (err) {
    console.error(`[slack-bridge] WebHost ingest failed: ${err.message}`);
  }
}
