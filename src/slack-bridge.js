 ...
 *   PULSENET_INGEST_URL - PulseNet ingest URL (default: http://localhost:3000)
 *   PULSENET_INGEST_TOKEN - PulseNet ingest bearer token
 */
...
const PULSENET_INGEST_URL    = process.env.PULSENET_INGEST_URL    || 'http://localhost:3000';
const PULSENET_INGEST_TOKEN  = process.env.PULSENET_INGEST_TOKEN  || '';
const SLACK_API        = 'https://slack.com/api';

// ---------------------------------------------------------------------------
// PulseNet ingest helper
// ---------------------------------------------------------------------------

async function ingestToPulseNet(msg) {
  if (!PULSENET_INGEST_TOKEN) {
    return;
  }
  try {
    const res = await fetch(`${PULSENET_INGEST_URL}/api/ingest`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${PULSENET_INGEST_TOKEN}`,
      },
      body: JSON.stringify(msg),
    });
    if (!res.ok) {
      console.error(`[slack-bridge] PulseNet ingest error: HTTP ${res.status}`);
    }
  } catch (err) {
    console.error(`[slack-bridge] PulseNet ingest failed: ${err.message}`);
  }
}
