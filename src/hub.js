...
// ── AgentPulse-to-Gateway Bridge Config ─────────────────────────────────────────────────
// On the AgentPulse (web-host) node: set AGENTPULSE_INGEST_URL to route agent
// replies back to AgentPulse UI instead of a non-existent AgentFramework gateway.
const AGENTPULSE_INGEST_URL   = process.env.AGENTPULSE_INGEST_URL   || '';
const AGENTPULSE_INGEST_TOKEN = process.env.AGENTPULSE_INGEST_TOKEN || '';
const GATEWAY_HOOK_URL      = process.env.GATEWAY_HOOK_URL      || 'http://127.0.0.1:18789/hooks/agent-pulse';
...
async function forwardToGateway(msg, agentName) {
  ...
  // ── Path A: AgentPulse ingest (web-host node — no local AgentFramework gateway) ──
  if (AGENTPULSE_INGEST_URL) {
    const ingestPayload = {
      from: sender,
      sender: sender,
      sender_type: 'agent',
    ...
    if (AGENTPULSE_INGEST_TOKEN) {
      headers['Authorization'] = `Bearer ${AGENTPULSE_INGEST_TOKEN}`;
    }

    try {
      const controller = new AbortController();
      const timeout = setTimeout(() => controller.abort(), 15000);
      const res = await fetch(AGENTPULSE_INGEST_URL, {
        method: 'POST',
        headers,
      ...
      console.log(`[pulse-bridge:ingest] Agent reply → AgentPulse ingest: ${res.status} (conv=${conversationId}, from=${sender}, len=${body.length})`);
      return null;  // ingest is fire-and-forget (no response body to relay)
    } catch (err) {
      console.warn(`[pulse-bridge:ingest] AgentPulse ingest failed: ${err.message} — falling through to gateway`);
      // Fall through to gateway path as backup
    }
  }

  // ── Path B: AgentFramework gateway hook (all other agent nodes) ──
  // Inject reply instructions so the agent sends response back to AgentPulse
  const replyInstructions = ""; // agent-pulse-relay hook handles routing automatically
  
  const hookPayload = {
    message: body + replyInstructions,
    sessionKey: `hook:agent-pulse:${conversationId}`,
    sender: sender,
    conversationId: conversationId,
  };
...
// ── End AgentPulse-to-Gateway Bridge ────────────────────────────────────────────────────
...
export class Hub extends EventEmitter {
  ...
  async _handleMeshDelivery(envelope) {
    ...
    // Application messages: forward to AgentFramework gateway (same as pulse/1.0 path)
  ...
  async _handleIncomingMessage(msg, fromPeer) {
    ...
    // Forward to local AgentFramework gateway via hook
  ...
  async _httpFallback(peerName, msg) {
    ...
    // HTTP POST to peer's AgentFramework hook endpoint (MESH v3 compatible).
